...
| `dift` | **Drag Item From To**: Zieht ein Diagramm-Item zu einem anderen Diagramm-Item. | `iif`/`bif`/`inf`: Index/Stack-Index/Name des zu ziehenden Items (Quelle). `iit`/`bit`/`int`: Index/Stack-Index/Name des Ziel-Items (wohin gezogen werden soll). |
...
| `clickRelationToolbarByItemNames` | **Click Relation Toolbar By Item Names**: Öffnet die Toolbar einer Relation, die durch die Namen ihrer Start- und End-Items identifiziert wird, und klickt dann auf einen Button in dieser Toolbar. |1) `startItemName`: Name des Start-Items der Relation.<br>2) `endItemName`: Name des End-Items der Relation.<br>3) `buttonSelector`: CSS-Selektor des Toolbar-Buttons. |
...
| `newRelationByItemNames` | **New Relation By Item Names**: Erstellt eine neue Relation zwischen zwei Items, indem es sie über ihre Namen identifiziert. Dies simuliert das Ziehen einer Verbindungslinie vom Start- zum Ziel-Item. |1) (Boolean) `ignoreTemplates` – legt fest, ob Templates ignoriert werden sollen.<br>2) `fromItemName`: Name des Start-Items.<br>3) `toItemName`: Name des Ziel-Items. |
...
| `editRelationByItemNames` | **Edit Relation By Item Names**: Bearbeitet die Eigenschaften einer Relation, die durch die Namen ihrer Start- und End-Items identifiziert wird. Öffnet den Bearbeitungsdialog und füllt die Felder aus. |1) `fromItemName`: Name des Start-Items der Relation.<br>2) `toItemName`: Name des End-Items der Relation.<br>3) `relationName`: (Optional) Neuer Name der Relation.<br>4) `showAsTable`: (Optional, Boolean) Steuert die Sichtbarkeit der Relation.<br>5) `backRelationName`: (Optional) Neuer Name der Rück-Relation.<br>6) `backShowAsTable`: (Optional, Boolean) Steuert die Sichtbarkeit der Rück-Relation.<br>7) `description`: (Optional) Neue Beschreibung der Relation.<br>8) `category`: (Optional) Neue Kategorie der Relation. |
...
| `setItemColor` | **Set Item Color**: Setzt die Farbe eines Items. |1) `itemIndex`: Item-Index des zu formatierenden Items (verwenden Sie `-1`, wenn Sie eine andere ID verwenden).<br>2) `stackIndex`: Stack-Index des zu formatierenden Items (verwenden Sie `-1`, wenn Sie eine andere ID verwenden).<br>3) `itemName`: Name des zu formatierenden Items (verwenden Sie `""`, wenn Sie eine andere ID verwenden).<br>4) `colorIndex`: Index der Farbe in der Farbpalette (0-basiert, z.B. `0` für die erste Farbe).<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true`, um das Schließen der Side Gallery zu überspringen. |
| `setItemFontSize` | **Set Item Font Size**: Setzt die Schriftgröße eines Items. |1) `itemIndex`: Item-Index des zu formatierenden Items (verwenden Sie `-1`, wenn Sie eine andere ID verwenden).<br>2) `stackIndex`: Stack-Index des zu formatierenden Items (verwenden Sie `-1`, wenn Sie eine andere ID verwenden).<br>3) `itemName`: Name des zu formatierenden Items (verwenden Sie `""`, wenn Sie eine andere ID verwenden).<br>4) `fontSizeSelector`: Selektor für die gewünschte Schriftgröße (z.B. `.id-font-size-small`, `.id-font-size-medium`, `.id-font-size-large`).<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true`, um das Schließen der Side Gallery zu überspringen. |
| `setRelationColor` | **Set Relation Color**: Setzt die Farbe einer Relation. |1) `fromItemIndex`: Item-Index des Start-Items der Relation.<br>2) `toItemIndex`: Item-Index des Ziel-Items der Relation.<br>3) `relationStackIndex`: Stack-Index der Relation.<br>4) `colorIndex`: Index der Farbe in der Farbpalette (0-basiert).<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true`, um das Schließen der Side Gallery zu überspringen. |
| `setRelationLineStyle` | **Set Relation Line Style**: Setzt den Linienstil einer Relation. |1) `fromItemIndex`: Item-Index des Start-Items der Relation.<br>2) `toItemIndex`: Item-Index des Ziel-Items der Relation.<br>3) `relationStackIndex`: Stack-Index der Relation.<br>4) `lineStyleSelector`: Selektor für den gewünschten Linienstil.<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true`, um das Schließen der Side Gallery zu überspringen. |
| `setRelationVisibleUpToLevel` | **Set Relation Visible Up To Level**: Stellt ein, bis zu welchem Level eine Relation sichtbar ist. |1) `fromItemIndex`: Item-Index des Start-Items der Relation.<br>2) `toItemIndex`: Item-Index des Ziel-Items der Relation.<br>3) `relationStackIndex`: Stack-Index der Relation.<br>4) `levelSelector`: Selektor für das gewünschte Sichtbarkeitslevel.<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true`, um das Schließen der Side Gallery zu überspringen. |
| `setTextNote` | **Set Text Note**: Fügt einem Item eine Textnotiz hinzu oder bearbeitet sie. |1) `itemIndex`: Item-Index des Items, zu dem die Notiz gehört (verwenden Sie `-1`, wenn Sie eine andere ID verwenden).<br>2) `stackIndex`: Stack-Index des Items, zu dem die Notiz gehört (verwenden Sie `-1`, wenn Sie eine andere ID verwenden).<br>3) `itemName`: Name des Items, zu dem die Notiz gehört (verwenden Sie `""`, wenn Sie eine andere ID verwenden).<br>4) `textContent`: Textinhalt der Notiz (HTML- oder Klartext).<br>5)-16) `tableParams` (optional): Parameter für das `insertTable`-Makro. |
