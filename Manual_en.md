...
| `dift` | **Drag Item From To**: Drags a diagram item to another diagram item. | `iif`/`bif`/`inf`: Index/Stack index/Name of the item to drag (source). `iit`/`bit`/`int`: Index/Stack index/Name of the target item (where to drag to). |
...
| `clickRelationToolbarByItemNames` | **Click Relation Toolbar By Item Names**: Opens the toolbar of a relation identified by the names of its start and end items, and then clicks a button within that toolbar. |1) `startItemName`: Name of the relation's start item.<br>2) `endItemName`: Name of the relation's end item.<br>3) `buttonSelector`: CSS selector of the toolbar button to be clicked. |
...
| `newRelationByItemNames` | **New Relation By Item Names**: Creates a new relation between two items by identifying them via their names. This simulates dragging a connection line from the start item to the target item. |1) (Boolean) `ignoreTemplates` – specifies whether templates should be ignored.<br>2) `fromItemName`: Name of the start item.<br>3) `toItemName`: Name of the target item. |
...
| `dragViewSliderBy` | **Drag View Slider By**: Drags a view slider (e.g., zoom, distance, rotation) by a specific amount. |1) `sliderSelector`: CSS selector of the slider (e.g., `.id-viewmode-zoom`).<br>2) `deltaX`: Pixels by which the slider should be moved (positive for right, negative for left).<br>3) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `resetViewSlider` | **Reset View Slider**: Resets a view slider to its default value. |1) `sliderSelector`: CSS selector of the slider (e.g., `.id-viewmode-zoom`).<br>2) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
...
| `editRelationByItemNames` | **Edit Relation By Item Names**: Edits the properties of a relation identified by the names of its start and end items. Opens the edit dialog and fills in the fields. |1) `fromItemName`: Name of the relation's start item.<br>2) `toItemName`: Name of the relation's end item.<br>3) `relationName`: (Optional) New name for the relation.<br>4) `showAsTable`: (Optional, Boolean) Controls the visibility of the relation.<br>5) `backRelationName`: (Optional) New name for the back-relation.<br>6) `backShowAsTable`: (Optional, Boolean) Controls the visibility of the back-relation.<br>7) `description`: (Optional) New description for the relation.<br>8) `category`: (Optional) New category for the relation. |
| `setZoom` | **Set Zoom**: Sets the zoom factor of the diagram. |1) `zoomValue`: Value for the zoom slider displacement (relative, e.g., `50` for half a displacement to the right).<br>2) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `resetZoom` | **Reset Zoom**: Resets the zoom factor of the diagram to its default value. |1) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setDistance` | **Set Distance**: Sets the distance between items in the diagram. |1) `distanceValue`: Value for the distance slider displacement (relative, e.g., `50`).<br>2) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `resetDistance` | **Reset Distance**: Resets the distance between items in the diagram to its default value. |1) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setRotation` | **Set Rotation**: Sets the rotation of the diagram (in 3D mode). |1) `rotationValue`: Value for the rotation slider displacement (relative, e.g., `50`).<br>2) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `resetRotation` | **Reset Rotation**: Resets the rotation of the diagram (in 3D mode) to its default value. |1) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setPerspective` | **Set Perspective**: Sets the perspective of the diagram (in 3D mode). |1) `perspectiveValue`: Value for the perspective slider displacement (relative, e.g., `50`).<br>2) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `resetPerspective` | **Reset Perspective**: Resets the perspective of the diagram (in 3D mode) to its default value. |1) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
...
| `setItemColor` | **Set Item Color**: Sets the color of an item. |1) `itemIndex`: Item index of the item to format (use `-1` if using another id).<br>2) `stackIndex`: Stack index of the item to format (use `-1` if using another id).<br>3) `itemName`: Name of the item to format (use `""` if using another id).<br>4) `colorIndex`: Index of the color in the color palette (0-based, e.g., `0` for the first color).<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setItemFontSize` | **Set Item Font Size**: Sets the font size of an item. |1) `itemIndex`: Item index of the item to format (use `-1` if using another id).<br>2) `stackIndex`: Stack index of the item to format (use `-1` if using another id).<br>3) `itemName`: Name of the item to format (use `""` if using another id).<br>4) `fontSizeSelector`: Selector for the desired font size (e.g., `.id-font-size-small`, `.id-font-size-medium`, `.id-font-size-large`).<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setRelationColor` | **Set Relation Color**: Sets the color of a relation. |1) `fromItemIndex`: Item index of the start item of the relation.<br>2) `toItemIndex`: Item index of the target item of the relation.<br>3) `relationStackIndex`: Stack index of the relation.<br>4) `colorIndex`: Index of the color in the color palette (0-based).<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setRelationLineStyle` | **Set Relation Line Style**: Sets the line style of a relation. |1) `fromItemIndex`: Item index of the start item of the relation.<br>2) `toItemIndex`: Item index of the target item of the relation.<br>3) `relationStackIndex`: Stack index of the relation.<br>4) `lineStyleSelector`: Selector for the desired line style.<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setRelationVisibleUpToLevel` | **Set Relation Visible Up To Level**: Sets the visibility level for a relation. |1) `fromItemIndex`: Item index of the start item of the relation.<br>2) `toItemIndex`: Item index of the target item of the relation.<br>3) `relationStackIndex`: Stack index of the relation.<br>4) `levelSelector`: Selector for the desired visibility level.<br>5) `skipCloseSideGallery`: (Optional, Boolean) `true` to skip closing the side gallery. |
| `setTextNote` | **Set Text Note**: Adds or edits a text note for an item. |1) `itemIndex`: Item index of the item the note belongs to (use `-1` if using another id).<br>2) `stackIndex`: Stack index of the item the note belongs to (use `-1` if using another id).<br>3) `itemName`: Name of the item the note belongs to (use `""` if using another id).<br>4) `textContent`: Text content of the note (HTML or plain text).<br>5)-16) `tableParams` (optional): Parameters for the `insertTable` macro. |
